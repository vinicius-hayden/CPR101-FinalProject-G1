...
void converting(void) {
    ...
    char longString[BUFFER_SIZE];  // Declare an array to store user input
    long longNumber;  // Declare a variable to store the converted number

    do {  // Start a do-while loop that will run at least once
        printf("Type the long numeric string (q - to quit):\n");  // Prompt user for input
        fgets(longString, BUFFER_SIZE, stdin);  // Read input from user (including newline)

        longString[strlen(longString) - 1] = '\0';  // Remove newline character by replacing with null terminator

        if (strcmp(longString, "q") != 0) {  // Check if input is not "q"
            longNumber = atol(longString);  // Convert string to long using atol
            printf("Converted number is %ld\n", longNumber);  // Print the converted number
        }
    } while (strcmp(longString, "q") != 0);  // Continue loop until user enters "q"
